...
fn main() {
    ...
    match jblock::parse_scenario(&scenario) {
        ...
        None => {
            eprintln!(
                "unknown scenario: {scenario:?}\nusage: gen_java <lock-contention|deadlock|thread-pool-exhaustion|sync-io-hotspot|dangerous-hot-lock|connection-pool-starve|future-latch-deadlock|logging-appender-contention|busy-wait-spin|condition-starvation|lock-order-risk|finalizer-pressure|sleep-as-scheduler|framework-pool-saturation> [count]"
            );
            exit(2);
        }
    }
