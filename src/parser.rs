...
"#;
    ...
    /// Java 8u45 jstack headers use HotSpot VM token `25.45-b02` (not Java 25).
    /// Version identification cannot be guaranteed, so Analysis must not emit it.
    #[test]
    fn does_not_identify_hotspot_vm_token_as_java_product_version() {
        let dump = concat!(
            "Full thread dump Java HotSpot(TM) 64-Bit Server VM (25.45-b02 mixed mode):\n",
            "\n",
            "\"main\" #1 prio=5 os_prio=0 tid=0x1 nid=0x1 runnable [0x1]\n",
            "   java.lang.Thread.State: RUNNABLE\n",
            "\tat demo.Main.main(Main.java:1)\n",
        );
        let a = analyze(dump);
        assert_eq!(a.format, DumpFormat::Jstack);
        assert_eq!(a.total_threads, 1);
        let json = serde_json::to_string(&a).expect("serialize Analysis");
        assert!(
            !json.contains("java_version"),
            "java_version must not be emitted: {json}"
        );
        assert!(
            !json.contains("25.45"),
            "HotSpot VM 25.45 must not leak into analysis JSON: {json}"
        );
    }
