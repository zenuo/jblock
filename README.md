...
- **问题模式**：死锁环、热锁竞争、**线程池耗尽**、**同步 I/O / RPC 热点簇** 等 Findings。
- **Java reproducer**：右上角 **Generate Java…** 打开模态框，生成可运行的死锁 / 锁竞争 / 线程池耗尽 / 同步 I/O 热点样例代码（不经过 WASM）；测试可用 JDK 实机 `jstack` 捕获 dump。
- **导出**：
