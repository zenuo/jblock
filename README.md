...
- **问题模式**：死锁环、热锁竞争、**线程池耗尽**、**同步 I/O / RPC 热点簇**、**危险热锁持有者（owner 阻塞调用）** 等 Findings。
- **Java reproducer**：右上角 **Generate Java…** 打开模态框，生成可运行的死锁 / 锁竞争 / 线程池耗尽 / 同步 I/O 热点 / 危险热锁样例代码（不经过 WASM）；测试可用 JDK 实机 `jstack` 捕获 dump。
- **导出**：
